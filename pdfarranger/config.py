...
_DEFAULT_ACCELS = [
    ...
    ('command-palette', '<Primary>k <Primary><Shift>p'),
    ('find', '<Primary>f'),
    ('find_prev', '<Shift>F3'),
    ('find_next', 'F3'),
]
...
class Config(object):
    ...
    def preferences_dialog(self, parent, handy_available):
        ...
        hbox = Gtk.Box(spacing=6, margin=3)
        frame = Gtk.Frame(label=_("Language"), margin=3)
        combo = Gtk.ComboBoxText(margin=3)
        ...
        hbox2 = Gtk.Box(spacing=6, margin=3)
        frame2 = Gtk.Frame(label=_("Theme"), margin=3)
        combo2 = Gtk.ComboBoxText(margin=3)
        ...
        frame3 = Gtk.Frame(label=_("Printing"), margin=3)
        ...
        if self.has_pikepdf8:
            frame4 = Gtk.Frame(label=_("Saving/exporting to single file"), margin=3)
            ...
            cb_retain = None
        frame5 = Gtk.Frame(label=_("Image Export"), margin=3)
        grid5 = Gtk.Grid(row_spacing=6, column_spacing=12, border_width=3)
        ...
        cb_optimize = Gtk.CheckButton(label=_("Optimize"))
        cb_optimize.set_active(self.optimize())
        grid5.attach(cb_optimize, 0, 2, 1, 1)
        cb_greyscale = Gtk.CheckButton(label=_("Greyscale"))
        ...
        frame6 = Gtk.Frame(label=t, shadow_type=Gtk.ShadowType.NONE, margin=3)
        label6 = Gtk.Label(self._config_file(self.domain), selectable=True, margin=3)
        frame6.add(label6)
